...
from CrystalConverter import Crystal2QWalkConverter
...
class LocalCrystalDFTQMC(Job):
  ...
  def __init__(self,jobid,struct,crystal_opts,structtype='cif'):
    ...
    crys_manager=mgmt.CrystalManager(
          cwriter,
          CrystalReader(),
          LocalCrystalRunner(),
          PropertiesReader(),
    ...
    # Better solution? Converter needs to know the CrystalManager's file names.
    converter=Crystal2QWalkConverter(**crys_manager.get_fileinfo())

    self.managers=[crys_manager,converter]
