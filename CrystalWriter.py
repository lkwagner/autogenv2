...
class CrystalWriter:
  def __init__(self,cif=None,xyz=None):
    ...
    #later
    if self.cif!=None:
      self.struct=CifParser.from_string(cif).get_structures(primitive=True)[0].as_dict()
      self.supercell=np.identity(3)
    elif self.xyz!=None:
      self.struct=XYZ.from_string(xyz).molecule.as_dict()['sites']
...
########################################################
  def crystal_input(self):
    ...
    if self.cif!=None: # Can also include 2d later.
