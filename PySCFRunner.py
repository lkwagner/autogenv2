...
class PySCFRunnerBW:
  ...
  def __init__(self,queue='batch',
    ...
    self.postfix=postfix
    self.exelines=[]
    if ppath is None: self.ppath=sys.path
    else:             self.ppath=ppath
    if prefix is None: self.prefix=[]
    else:              self.prefix=prefix
    if postfix is None: self.postfix=[]
  ...
  def script(self,scriptfile):
    ...
    # Prepend mp specs.
    exelines=["export OMP_NUM_THREADS=%d"%(self.nn*self.np)]
    for line in self.exelines:
      exelines+=["aprun -N 1 -n {nn} -d {np} -b -- {line}"\
          .format(nn=self.nn,np=self.np,line=line)]
