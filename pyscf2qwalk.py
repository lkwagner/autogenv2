...
############################################################
def binary_to_str(S, norb):
    occup= [str(i+1)  for i, c in enumerate(reversed(S)) 
            if c=='1' and i< norb]
    return ' '.join(occup) 

def print_cas_slater(mc,orbfile, basisfile,f, tol = .01):
    norb  = mc.ncas 
    nelec = mc.nelecas
    ...
    for x in deters:
        ...
        tmp= ("# spin up orbitals \n"+ binary_to_str(x[1], norb) 
             + "\n#spin down orbitals \n" + binary_to_str(x[2], norb)) 
        occup.append(tmp) 
    det = ' '.join(detwt) 
    occ =  '\n\n'.join(occup) 
  ...
  STATES {
  %s 
  }''' %(orb_type, norb, orbfile, basisfile, det, occ))
    f.close()
    return 
