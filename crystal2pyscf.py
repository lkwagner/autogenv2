...
def crystal2pyscf_cell(propoutfn="prop.in.o",
  ...
  mf=check_mf
  print(check_mf.mo_energy)
  crydf=format_eigenstates_cell(cell,cryeigsys)
  mf.mo_energy=cryeigsys['eigvals']
  mf.mo_coeff=[crydf[[*range(crydf.shape[0])]].values]
  #mf.mo_occ=[cryeigsys['eig_weights'][0][0]*2]
  print(mf.mo_energy)
...
##########################################################################################################
def format_eigenstates_cell(cell,cryeigsys):
  ''' Organize crystal eigenstates to be consistent with PySCF order.
...
##########################################################################################################
def test_crystal2pyscf_cell(ref_chkfile="pyscf_driver.py.o",propoutfn="prop.in.o"):
  ''' Test converter by reading an identical PySCF run and checking results are consistent.'''
  #TODO Doc
  ...
  # Result of conversion.
  cell,mf=crystal2pyscf_cell(basis=basis,gs=(8,8,8))
...
if __name__=='__main__':
  test_crystal2pyscf_cell(ref_chkfile="../pyscf/pyscf_driver.py.chkfile")
